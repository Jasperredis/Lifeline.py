...
# Import modules
import assets
from assets import ld_imgc, ld_audc, unld_aud, unld_img
...
def makeDisp():
    global screen, rsurface, winsize, winscale, base_winsize
    if window == "b":  # b stands for borderless
        base_winsize = (256, 144)
        winscale = 4
        winsize = (base_winsize[0] * winscale, base_winsize[1] * winscale)
        screen = pygame.display.set_mode(winsize)
        rsurface = pygame.Surface(base_winsize)
...
updt_font = pygame.font.SysFont("Arial", 12)
updd_font = pygame.font.SysFont("Arial", 8)
updv_font = pygame.font.SysFont("Arial", 12)
...
def enemy():
    ex_x = random.randint(37, 216)
    ex_y = rsurface.get_height() // 2
    enemies.append([ex_x, ex_y])

def heal():
    h_x = random.randint(37, 216)  # Random x position
    h_y = rsurface.get_height() // 2
...
def dispLns():
    ...
    lny = 64  # Starting vertical position for text
    ...
    while lnscan <= total_lines:
        ...
        if line_key in current_page and isinstance(current_page[line_key], str):
            ...
            rsurface.blit(text_surface, (46, lny))
        else:
            print(f"Skipping line {line_key}: invalid or missing text in page {pg}.")
        
        lny += 9  # Increment vertical position for the next line
        lnscan += 1
...
def ingnotif(imgt, imgf, pos): # pos can be tl (top left), tr (top right), bl (bottom left), or br (bottom right)
    ...
    if pos == "tl" or pos == "tr":
        ...
        notif_rect.y = 555
    if notif_rect.collidepoint(mx, my):
        rsurface.blit(imgt, (notif_rect))
        if mb[0]:
            return True
        else:
            return False
    else:
        rsurface.blit(imgf, (notif_rect))
        return False
...
while running:
        ...
        rsurface.fill((fillc))  # Finally, fill the screen with the chosen colour

        text_surface = fpfont.render(f"FPS: {fps}", True, (255, 255, 255))
        rsurface.blit(text_surface, (730, 0))
    ...
    # *Render background
        ...
        bgrect.centerx = rsurface.get_width() // 2
        bgrect.y = 7
        if not state == "intro" and not area == 2:  # Allows the black background in the intro
            if bg == 1:
                rsurface.blit(assets.bg_r, (bgrect))
            elif bg == 2:
                rsurface.blit(assets.bg_o, (bgrect))
            elif bg == 3:
                rsurface.blit(assets.bg_y, (bgrect))
            elif bg == 4:
                rsurface.blit(assets.bg_g, (bgrect))
            elif bg == 5:
                rsurface.blit(assets.bg_b, (bgrect))
            elif bg == 6:
                rsurface.blit(assets.bg_pu, (bgrect))
            elif bg == 7:
                rsurface.blit(assets.bg_pi, (bgrect))
            elif bg == 8:
                rsurface.blit(assets.bg_gg, (bgrect))
            elif bg == 9:
                rsurface.blit(assets.bg_db, (bgrect))
            elif bg == 10:
                rsurface.blit(assets.bg_mg, (bgrect))
            elif bg == 11:
                rsurface.blit(assets.bg_gr, (bgrect))
        if area == 2:
            rsurface.blit(assets.bg_opts , (bgrect))

        # *Intro loop
        if state == "intro":  #! Possibly not done yet
            # Enable alpha transparency if not already set
            rjriimg = pygame.Surface(assets.jris_img.get_size(), pygame.SRCALPHA)  # Create a transparent surface
            rjriimg.blit(assets.jris_img, (0, 0))  # Blit the image to the transparent surface
            ...
            if intropart == 1:
                ...
                if not introtick >= 150:
                    rotated_img = pygame.transform.rotate(rjriimg, jrot)  # Rotate the image
                    # Get a rect centered around a fixed position
                    img_rect = rotated_img.get_rect(center=(rsurface.get_width() // 2, rsurface.get_height() // 2))
                    # Clear the surface before drawing (for smooth updates)
                    rsurface.fill((0, 0, 0, 0))  # Clear with transparency
                    # Blit the rotated image
                    rsurface.blit(rotated_img, img_rect)
                ...
                if introtick >= 150:  # Begin second part if 150 ticks have passed
                    ...
                    jtxt = pygame.transform.scale(jtxt, (142, 36))
                    if jx > 20:  # Ensures jx doesn’t overshoot
                        jx_div = 2
                        jx -= jx_div
                    else:
                        jx = 20  # Lock jx at the target position once minimum is reached
                    img_rect = rjriimg.get_rect()
                    img_rect.x = jx
                    img_rect.centery = rsurface.get_height() // 2
                    rsurface.blit(rjriimg, (img_rect.x, img_rect.y))
                    rsurface.blit(jtxt, (40, 55))
                if introtick == 340:
                    intropart = 2
                    introtick = 0
            else:
                ...
                img_rect = proudlypresents.get_rect(center=(rsurface.get_width() // 2, rsurface.get_height() // 2))
                rsurface.blit(proudlypresents, (img_rect))
                if introtick >= 200:
                    state = "title"
        ...
        if state == "title":
            ...
            # if mute:
            #     rsurface.blit(musicf, (music_rect))  # Mute icon
            # else:
            #     rsurface.blit(musict, (music_rect))  # Unmute icon

            # *Render buttons
            pygame.display.set_caption("Lifeline.py - Title Screen")
            if not area == 2:
                img_rect = assets.lll_img.get_rect()
                img_rect.centerx = rsurface.get_width() // 2
                img_rect.y = 7
                rsurface.blit(assets.lll_img, (img_rect))
            if area == 1:  # 1 is the main menu screen
                ...
                img_rect.y = 55
                if sel == 1:
                    rsurface.blit(assets.start_t_img, (img_rect))
                else:
                    rsurface.blit(assets.start_f_img, (img_rect))
                # Render options button
                img_rect = assets.optt_img.get_rect()
                img_rect.centerx = rsurface.get_width() // 2
                img_rect.y = 62
                if sel == 2:
                    rsurface.blit(assets.optt_img, (img_rect))
                else:
                    rsurface.blit(assets.optf_img, (img_rect))
                # Render updates button
                img_rect = assets.updatet_img.get_rect()
                img_rect.centerx = rsurface.get_width() // 2
                img_rect.y = 69
                if sel == 3:
                    rsurface.blit(assets.updatet_img, (img_rect))
                else:
                    rsurface.blit(assets.updatef_img, (img_rect))
                # Render update tick if not on latest version
                if not latestVer:
                    rsurface.blit(assets.upd_ping, (478, 326))
                # Render difficulty
                ...
                if sel == 4:
                    if dif == 1:
                        rsurface.blit(assets.dif_1_t_img, (img_rect))
                    elif dif == 2:
                        rsurface.blit(assets.dif_2_t_img, (img_rect))
                    elif dif == 3:
                        rsurface.blit(assets.dif_3_t_img, (img_rect))
                else:
                    if dif == 1:
                        rsurface.blit(assets.dif_1_f_img, (img_rect))
                    elif dif == 2:
                        rsurface.blit(assets.dif_2_f_img, (img_rect))
                    elif dif == 3:
                        rsurface.blit(assets.dif_3_f_img, (img_rect))
            ...
            # Render background colour button
                ...
                img_rect.y = 270
                if sel == 1:
                    rsurface.blit(assets.bg_t_img, (img_rect))
                else:
                    rsurface.blit(assets.bg_f_img, (img_rect))
                # Render controls button
                img_rect = assets.controls_t.get_rect()
                img_rect.centerx = rsurface.get_width() // 2
                img_rect.y = 300
                if sel == 2:
                    rsurface.blit(assets.controls_t, (img_rect))
                else:
                    rsurface.blit(assets.controls_f, (img_rect))
                # Render back button
                img_rect = assets.back_t_img.get_rect()
                img_rect.centerx = rsurface.get_width() // 2
                img_rect.y = 330
                if sel == 3:
                    rsurface.blit(assets.back_t_img, (img_rect))
                else:
                    rsurface.blit(assets.back_f_img, (img_rect))
                ...
                if not latestVer: # Display update version if not on the latest version
                    ...
                    if updb_rect.collidepoint(mx, my): # Check if mouse is touching hitbox:
                        ...
                        if mb[0] and (current_time - last_key_prestime >= key_delay): # Avoid repeatedly triggering:
                            ...
                            last_key_prestime = current_time
                    rsurface.blit(assets.upd_button, (updb_rect))
                #* Add clickable buttons
                refb_rect = assets.refb.get_rect() # Display and make refresh button work
                if latestVer:
                    refb_rect.centerx = rsurface.get_width() // 2 + 72
                else:
                    refb_rect.centerx = rsurface.get_width() // 2 + 199
                refb_rect.y = 398
                if refb_rect.collidepoint(mx, my): # Check if mouse is touching hitbox
                    refb_rect.y = 395
                    if mb[0] and (current_time - last_key_prestime >= key_delay): # Avoid repeatedly triggering
                        connection = isconnected()
                        if connection:
                            ...
                            if float(upd['version']) == version and connection:
                                ...
                                if not connection: # Tell the game that it's on the latest version so that the update button doesn't display with no connection
                                    ...
                                    latestVer = False
                        last_key_prestime = current_time
                rsurface.blit(assets.refb, (refb_rect))
                
                prev_rect = assets.prev_pg.get_rect() # Display and make previous page button work
                if latestVer:
                    prev_rect.centerx = rsurface.get_width() // 2 - 161
                else:
                    prev_rect.centerx = rsurface.get_width() // 2 - 317
                prev_rect.y = 398
                if prev_rect.collidepoint(mx, my): # Check if mouse is touching hitbox:
                    prev_rect.y = 395
                    if mb[0] and (current_time - last_key_prestime >= key_delay): # Avoid repeatedly triggering
                        pg -= 1
                        if pg < 1:
                        ...
                        last_key_prestime = current_time
                rsurface.blit(assets.prev_pg, (prev_rect))

                next_rect = assets.next_pg.get_rect() # Display and make next page button work
                if latestVer:
                    next_rect.centerx = rsurface.get_width() // 2 + 161
                else:
                    next_rect.centerx = rsurface.get_width() // 2 + 317
                next_rect.y = 398
                if next_rect.collidepoint(mx, my): # Check if mouse is touching hitbox
                    next_rect.y = 395
                    if mb[0] and (current_time - last_key_prestime >= key_delay): # Avoid repeatedly triggering:
                        pg += 1
                        if pg > upd['pgs']:
                        ...
                        assets.move_sel_sfx.play()
                        last_key_prestime = current_time
                rsurface.blit(assets.next_pg, (next_rect))
                ...
                if latestVer:
                    bacb_rect.centerx = rsurface.get_width() // 2 - 72
                else:
                    bacb_rect.centerx = rsurface.get_width() // 2 - 199
                bacb_rect.y = 398
                if bacb_rect.collidepoint(mx, my): # Check if mouse is touching hitbox:
                    bacb_rect.y = 395
                    if mb[0]:
                        if lazyload:
                            ld_imgc("tts1")
                        ...
                        rsurface.blit(assets.bacb, (bacb_rect))
                else:
                    rsurface.blit(assets.bacb, (bacb_rect))
                ...
                rsurface.blit(text_surface, (85, 235))
                ...
                rsurface.blit(text_surface, (85, 255))
                ...
                rsurface.blit(text_surface, (85, 275))
                ...
                rsurface.blit(text_surface, (85, 295))
                ...
                rsurface.blit(text_surface, (85, 315))
                ...
                rsurface.blit(text_surface, (85, 335))
                ...
                rsurface.blit(text_surface, (675, 175))
                if sel == 2: # Move Right
                ...
                rsurface.blit(text_surface, (675, 195))
                if sel == 3: # Pause
                ...
                rsurface.blit(text_surface, (675, 215))
                if sel == 4: # Navigate up in menus
                ...
                rsurface.blit(text_surface, (675, 235))
                if sel == 5: # Navigate down in menus
                ...
                rsurface.blit(text_surface, (675, 255))
                if sel == 6: # Navigate left in menus
                ...
                rsurface.blit(text_surface, (675, 275))
                if sel == 7: # Navigate right in menus
                ...
                rsurface.blit(text_surface, (675, 295))
                if sel == 8: # Select in title screen menus
                ...
                rsurface.blit(text_surface, (675, 315))
                if sel == 9: # Select in pause menu
                ...
                rsurface.blit(text_surface, (675, 335))
                #* Display Help Option
                if sel == 10:
        ...
        if state == "game":
            ...
            img_rect.centerx = rsurface.get_width() // 2
            img_rect.centery = rsurface.get_height() // 2
            rsurface.blit(assets.bar_img, (img_rect))
            ...
            plr_rect.x = plrx
            plr_rect.centery = rsurface.get_height() // 2
            rsurface.blit(assets.plr_img, (plr_rect))
            ...
            img_rect.y = 5
            if protection == True:
                rsurface.blit(assets.protected_eff, img_rect)

            # Life display script
            if life < 1:
                img_rect = assets.gameover_img.get_rect()
                img_rect.centerx = rsurface.get_width() // 5
                img_rect.y = rsurface.get_height() // 2 + 5
                rsurface.blit(assets.gameover_img, (img_rect))
            else:
                img_rect = assets.L5_img.get_rect()
                img_rect.centerx = rsurface.get_width() // 5
                img_rect.y = rsurface.get_height() // 2 + 5
                if life == 5:
                    rsurface.blit(assets.L5_img, (img_rect))
                elif life == 4:
                    rsurface.blit(assets.L4_img, (img_rect))
                elif life == 3:
                    rsurface.blit(assets.L3_img, (img_rect))
                elif life == 2:
                    rsurface.blit(assets.L2_img, (img_rect))
                elif life == 1:
                    rsurface.blit(assets.L1_img, (img_rect))
            ...
            for en in enemies:
                ex, ey = en
                rsurface.blit(assets.enemy_img, (ex, ey))
            ...
            for h in heals:
                hx, hy = h
                rsurface.blit(assets.heal_img, (hx, hy))
            ...
            if paused == True and gameover == False:
                img_rect.centerx = rsurface.get_width() // 10
                img_rect.y = 560
                rsurface.blit(assets.pause_barcontent, (img_rect))
                # Make menu
                img_rect = assets.pause_1_img.get_rect()
                img_rect.centerx = rsurface.get_width() // 2
                img_rect.centery = rsurface.get_height() // 2
                if sel == 1:
                    rsurface.blit(assets.pause_1_img, (img_rect))
                elif sel == 2:
                    rsurface.blit(assets.pause_2_img, (img_rect))
                elif sel == 3:
                    rsurface.blit(assets.pause_3_img, (img_rect))
            ...
            # Gameover script
            if gameover == True:
                rsurface.blit(assets.gameover_barcontent, (20, 560))
        ...
        if autosaving == True:
            if autosavei == 1:
                rsurface.blit(assets.autosave1, (5, 5))
            elif autosavei == 2:
                rsurface.blit(assets.autosave2, (5, 5))
            elif autosavei == 3:
                rsurface.blit(assets.autosave3, (5, 5))
            elif autosavei == 4:
                rsurface.blit(assets.autosave4, (5, 5))
            elif autosavei == 5:
                rsurface.blit(assets.autosave5, (5, 5))
            elif autosavei == 6:
                rsurface.blit(assets.autosave6, (5, 5))
            elif autosavei == 7:
                rsurface.blit(assets.autosave7, (5, 5))
            elif autosavei == 8:
                rsurface.blit(assets.autosave8, (5, 5))

        if my < 45: # Show X and fullscreen/minimize if mouse is at top
            xhb = assets.xt_img.get_rect() #* Make for X button
            xhb.x = rsurface.get_width() // 2 - 3 #XHB stands for X HitBox
            xhb.y = 0
            if xhb.collidepoint(mx, my): # Check if mouse is touching hitbox
                rsurface.blit(assets.xt_img, (xhb))
                if mb[0]: # Check if mouse is down
                    byebye() # End the program
            else:
                rsurface.blit(assets.xf_img, (xhb))
            fmhb = assets.mt_img.get_rect() #* Make rect for minimize/fullscreen button
            fmhb.x = rsurface.get_width() // 2 + 3 #FMHB stands for Fullscreen/Minimize HitBox
            fmhb.y = 0
            if fmhb.collidepoint(mx, my): # Check if mouse is touching hitbox
                if window == "b": # Check if window is bordered
                    rsurface.blit(assets.ft_img, (fmhb))
                    ...
                    rsurface.blit(assets.mt_img, (fmhb))
                    if mb[0] and (current_time - last_key_prestime >= key_delay): # Check if mouse is down and avoid repetitive presses
                        window = "b"
                        makeDisp() # Refresh window
                        last_key_prestime = current_time # Reset last key press
                ...
                if window == "b": # Check if window is bordered
                    rsurface.blit(assets.ff_img, (fmhb))
                else: # Else if window is fullscreen
                    rsurface.blit(assets.mf_img, (fmhb))
        play_music()

        #* Final rendering essentials
        rsurface.blit(assets.mouse_img, (mx, my)) # Render mouse
        # Scale the render surface to fit the user-visible window size
        scaled_surface = pygame.transform.scale(rsurface, winsize)
